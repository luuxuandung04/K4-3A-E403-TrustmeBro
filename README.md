...
* **Lát cắt MỘT CÂU:**  
  > *Một câu hỏi → Một quyết định logic → Một câu trả lời chính xác có trích dẫn nguồn.*  
  > *(Học viên hỏi hạn nộp bài trên Discord · AI bot phân loại intent và chỉ trích xuất từ thông báo chính thức, nếu thiếu thông tin sẽ từ chối bịa và tag TA trực ca · học viên nhận đúng deadline và không bị mất điểm oan).*
* **Luồng xử lý cốt lõi (User Flow):**  
  `Học viên hỏi deadline / link nộp / quy chế` → `AI nhận diện intent, tuần học, đối tượng` → `Đối chiếu kho dữ liệu thông báo chính thức mới nhất` → `Trả lời ngắn gọn kèm link nguồn đối chứng`.
* **Bộ quy tắc phản hồi (Guardrails):**
  1. **FOUND (Đủ căn cứ):** Tìm thấy thông báo chính thức còn hiệu lực → Trả lời trực tiếp hạn chót/link + đính kèm trích dẫn (link tin nhắn gốc / kênh `#announcement`).
  2. **CLARIFY (Thiếu dữ kiện):** Câu hỏi chưa rõ tên bài lab/quiz/module → Phản hồi nhanh yêu cầu học viên chọn đúng đợt bài cần tra cứu (HAX G10).
  3. **NOT FOUND (Chưa có thông báo):** Không có nguồn xác thực → Tuyệt đối không bịa đặt (**Zero Hallucination**), thông báo rõ ràng thông tin chưa được công bố và cung cấp nút tag/liên hệ TA hỗ trợ (HAX G1, G8).
* **Cơ chế Automation:** `Conditional Retrieval` kết hợp kiểm tra tính hợp lệ của liên kết (Link Validity Check) trước khi trả về cho người dùng.
...
- [ ] **CP2 (21:00 · 16/9):** Clickable Prototype bấm thông suốt luồng chính + Commit đầu tiên trên GitHub.
...
*Tài liệu hướng dẫn chi tiết dành cho bạn Lan Anh và nhóm: xem tại [`HUONG_DAN_PROTOTYPE_CHO_LAN_ANH.md`](./HUONG_DAN_PROTOTYPE_CHO_LAN_ANH.md).*
