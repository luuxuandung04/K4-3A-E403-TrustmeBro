...
from backend.config import DEFAULT_TIMEZONE, GEMINI_MODEL, WHITELIST_CHANNELS
...
def process_discord_event(event: DiscordRawEvent) -> Dict[str, Any]:
    ...
    raw_channel = event.d.channel_id.replace("chan_", "").replace("#", "").strip()
    ch_clean = WHITELIST_CHANNELS.get(
        event.d.channel_id,
        raw_channel.replace("_", "-")
    )
