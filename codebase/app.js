...
async function jumpToSourceMessage(rawChannel, rawMsgId, contextKw = "") {
  ...
  if (!switchChannel(targetChannel, false)) {
    showToast(`⚠️ Không tìm thấy kênh nguồn #${targetChannel}.`);
    return false;
  }

  await waitForFeedLayout();
  let sourceMessage = findRenderedSourceMessage(targetMsgId, contextKw);

  // Fallback: first non-bot message in this channel
  if (!sourceMessage) {
    const nonBotMsgs = messageFeed.querySelectorAll(".discord-message:not(.bot-discord-message)");
    if (nonBotMsgs.length > 0) {
      sourceMessage = nonBotMsgs[0];
    }
  }
