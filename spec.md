...
* **Phần chạy thật trong CP2:** Hội thoại cuộn độc lập; lệnh `/deadline tong-hop`, `/deadline xem`, `/deadline them`; calendar nằm trong bot message; nút chuyển tháng/chi tiết/đồng bộ; admin modal; bốn nhánh quyết định; correction và decision trace chạy bằng HTML/CSS/JavaScript. Tin nhắn mới được lưu/nạp lại theo channel qua API cục bộ và `codebase/data/mes.json`.
* **Phần giả lập trong CP2:** Discord Gateway/API, Message Content Intent, Components V2, role permission, model trích xuất JSON và database/audit log của deadline là mock. File JSON chỉ thay database cho lịch sử chat demo; prototype không gửi dữ liệu sang Google Calendar hoặc website ngoài.
